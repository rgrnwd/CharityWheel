...
describe('Physics', function() {
    ...
    describe('getSelectedcharityIndex', function(){

    	it('should select the first charity when it is in the first arc and angled at marker', function(){
    		var totalArcs = 4;
    		var startRadian = 190 * Math.PI / 180;
    		var charities = [{'votes': 1}, {'votes': 1}, {'votes': 1}, {'votes': 1}]
    		var index = physics.getSelectedcharityIndex(charities, totalArcs, startRadian);
    		assert.equal(0, index);
    	});

    	it('should select the correct charity when there are more arcs than charities', function(){
    		var totalArcs = 4;
    		var startRadian = 190 * Math.PI / 180;
    		var charities = [{'votes': 2}, {'votes': 1}, {'votes': 1}]
    		var index = physics.getSelectedcharityIndex(charities, totalArcs, startRadian);
    		assert.equal(0, index);
    	});

    	it('should select the correct charity when it is angled at marker', function(){
    		var totalArcs = 8;
    		var startRadian = 240 * Math.PI / 180;
    		var charities = [{'votes': 2}, {'votes': 1}, {'votes': 2}, {'votes': 2}, {'votes': 1}]
    		var index = physics.getSelectedcharityIndex(charities, totalArcs, startRadian);
    		assert.equal(0, index);
    	});

    	it('should select the correct charity when it is angled at another marker', function(){
    		var totalArcs = 12;
    		var startRadian = 30 * Math.PI / 180;
    		var charities = [{'votes': 2}, {'votes': 1}, {'votes': 3}, {'votes': 1}, {'votes': 5}]
    		var index = physics.getSelectedcharityIndex(charities, totalArcs, startRadian);
    ...
    describe('getcharityFromSelectedArc', function(){

    	it('should return -1 if no charities', function(){
    		var charities = [];
    		var index = physics.getcharityFromSelectedArc(charities, 4);
    		assert.equal(-1, index);
    	});

    	it('should return last charities if list of charities is greater than arc given', function(){
    		var charities = [{'votes':1}];
    		var index = physics.getcharityFromSelectedArc(charities, 4);
    		assert.equal(0, index);
    	});

    	it('should return selected charity if arcs and charities are the same length', function(){
    		var charities = [{'votes':1}, {'votes':1}, {'votes':1}];
    		var index = physics.getcharityFromSelectedArc(charities, 2);
    		assert.equal(2, index);
    	});

    	it('should return second charity if 3rd arc is given and first charity equals two arcs', function(){
    		var charities = [{'votes':2}, {'votes':1}];
    		var index = physics.getcharityFromSelectedArc(charities, 2);
    		assert.equal(1, index);
    	});

    	it('should return first charity if 3rd arc is given and first charity equals three arcs', function(){
    		var charities = [{'votes':3}, {'votes':1}];
    		var index = physics.getcharityFromSelectedArc(charities, 2);
    		assert.equal(0, index);
    	});

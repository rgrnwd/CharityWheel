...
function initWheel(context, scaleFactor, charities, colors, startAngle) {
    setCanvasSize(context, scaleFactor);
    setContextStyle(context, '22' * scaleFactor);
    drawWheel(context, charities, colors, startAngle)
}
...
function drawWheel(context, charities, colors, startAngle) {
    var center = PhysicsCenter;

    var colorIndex = 0;
    var totalWeight = votes.getTotalVotes(charities);
    ...
    for(var i = 0; i < charities.length; i++) {
        ...
        var charityName = charities[i].name;

        var charityVotes = charities[i].votes;
        var arcSize = arc * charityVotes;
        ...
        if (charityVotes != 0) {
    		...
    		drawHighlightedText(context, charityName, 35, 7, 'white', 'black');
            colorIndex++;
        }

		accumulatedWeight += charityVotes;
        context.restore();
    }
}
